...
<!-- 以下のTODOセクションは map 実行時に自動で埋まる。テンプレート状態では空で正常。 -->

## プロジェクト概要
<!-- TODO: map で自動生成、または会話で伝えてください -->

## 技術スタック
<!-- TODO: map で自動検出して記入する。手動記入も可。 -->
...
## デザイン方針
<!-- mock / design / build(coder) が UI 作成時に参照する視覚の既定。未記入のまま UI 要件が来たら grill / design が確定を求める -->
<!-- 例: -->
<!-- - 用途: 管理画面 = 使いやすさ優先（装飾より情報密度・操作効率） -->
<!-- - レスポンシブ: 対応する（特記なければ既定） -->
<!-- - ブランドカラー: #2563eb / フォント: システムフォント -->
<!-- - デザイントークン確定後は docs/design/design-system.md が真実（design スキル成果物） -->
...
## テスト設定
<!-- TODO: map で自動生成、またはプロジェクトに合わせて記入 -->
...
### CI外部サービス
<!-- TODO: プロジェクトを分析して自動記入する -->
<!-- CIのE2Eテストで必要な外部サービスの起動方法。 -->
<!-- 例: -->
