...
リリース前の追加機能・改修は、正本（`docs/`）へ入れる前に `pre-specs/` に要件・設計をためる。`docs/` は公開済みの正本、`pre-specs/` は実装前の下書きで非公開。本番リリース時に Spec ID を採番して `docs/` へ昇格させる。

- `docs/`: リリース済み機能の正本（MkDocs 等で公開）
- `pre-specs/`: 実装前の要件定義・基本設計（非公開）

このフローは任意。導入しないプロジェクトは従来どおり `docs/` に直接 Spec を作成してよい。導入する場合のみ以下の構造・昇格手順に従う。

### pre-specs/ 構造

```
pre-specs/features/<機能名（kebab-case）>/
  ├── req-<機能名>.md      ← 要件定義（WHAT/WHY）
  └── design-<機能名>.md   ← 基本設計（HOW）
```

設計書ファースト原則は `pre-specs/` も対象（コード変更前に `pre-specs/` の設計書を更新する）。

### docs/ への昇格

リリース時に正本へ昇格させる：

1. Spec ID（`REQ-[機能]-[連番]`）を採番する（ID 体系は本ルール冒頭に従う）
2. `pre-specs/features/<機能名>/` の内容を `docs/requirements/`（必要に応じて `docs/design/`）の該当パスへ移動する
3. `spec-map.yml` にエントリを追加する（`spec_version: 1, confirmed: 1`）
4. MkDocs を使う場合は nav（`mkdocs.yml`）に追加する
5. 元の `pre-specs/features/<機能名>/` を削除する
...
- **形式:** `相対パス:シンボル名`（関数・メソッド・クラス名）を既定とする（例: `src/auth/signup.ts:signup`）
- パスはコードリポのルートからの相対パス
- 行番号は壊れやすい（上流に1行追加するだけで全ての引用がずれる）ため既定にしない。シンボル名は関数の移動・行追加では壊れず、リネーム時のみ更新が必要
- **行番号は補助**：特定の行を厳密に指す必要がある場合に限り、シンボル名に併記する（例: `src/auth/signup.ts:signup` 内のメール検証行）
- spec-map の `path` 形式（`相対パス:関数名`）と揃える
