...
- **ガードレール自体の無効化（自己防衛・hooks でブロック済み）**: `--no-verify`（短縮形 `git commit -n` 含む）によるフック迂回 / `git config core.hooksPath` の変更 / `.claude/hooks`・`.claude/settings*.json` の Bash 経由改変（`rm`/`sed -i`/`truncate`/`mv`/`tee`/リダイレクト）/ `.claude/hooks` への `chmod`。ガードレールの変更は**人間が Edit/Write でレビュー可能な形で**行う（`principles.md` §11「絶対禁止」）
...
- **自己防衛（絶対禁止）**: `--no-verify`（短縮形 `git commit -n` 含む）/ `core.hooksPath` 変更 / `.claude/hooks`・`settings` の Bash 改変 / hooks への `chmod`
