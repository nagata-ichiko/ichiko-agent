...
**環境によってはステンシルレジストリに登録されておらず**、ただの黒い四角形になる。
draw.io 本体やエクスポート（PNG/SVG）で確実に描画させるには、複合シェイプを避けて直接ステンシルを参照する。

**直接ステンシルを参照する**:
```
shape=mxgraph.aws4.cloudfront     ← OK（確実に描画される）
